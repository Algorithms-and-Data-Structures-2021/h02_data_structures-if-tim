...
namespace itis {
    ...
    void LinkedList::Insert(int index, Element e) {
        ...
        if (index != 0 && index != size_) {
            new_node->next = find_node(index);
            find_node(index - 1)->next = new_node;
        }
    }
    ...
    Element LinkedList::Remove(int index) {
        ...
        // Tip 2: используйте функцию find_node(index) отличается от правильной версии на -1
        // напишите свой код здесь ...
        ...
        if (index == 0) {
            removed_node = head_;
            removed_data = head_->data;
            head_ = head_->next;
            delete removed_node;
            size_ - +1;
            return removed_data;
        }
        Node *before_removed_node = find_node(index - 1);
        removed_node = before_removed_node->next;
        before_removed_node->next = removed_node->next;
